...
async def cmd_panel(msg: Message, state: FSMContext):
    ...
    await show_menu(msg, text, admin_panel_kb(is_owner=owner, bot_enabled=bot_on), with_photo=True)
...
async def show_queue(call: CallbackQuery):
    ...
    for row in rows:
        ...
        text += (
            ...
            f"<b>От:</b> {('@'+username) if username else 'Отсутствует'} (<code>{user_id}</code>)\n"
            f"<b>Категория:</b> {cl}\n"
            f"Статус: {st}\n"
        )
        if code:
...
async def clear_queue_do(call: CallbackQuery):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        f"Удалено пользовательских заявок: {deleted}"
    )
...
async def set_price_new_process(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        tg(T_OK, "✅") + " <b>Цена обновлена</b>\n━━━━━━━━━━━━━━━━\n"
        f"MAX • Нерег: <code>${price:.2f}</code>",
        reply_markup=back_to_admin_kb(), parse_mode="HTML"
    )
    await state.clear()
...
async def show_stats(call: CallbackQuery):
    ...
    text = (
        ...
        f"├ {tg(T_ERR, '❌')} <b>Неуспешных:</b> {rejected}\n"
...
async def broadcast_process(msg: Message, state: FSMContext, bot: Bot):
    ...
    for (user_id,) in users:
        # Текст рассылки только обычным пользователям с доступом — не админам и не владельцу
...
async def users_list(call: CallbackQuery):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        f"Всего активных: <code>{active_count}</code>"
    )
    ...
    for row in rows:
        ...
        try:
            await call.message.answer(
                ...
                f"<b>Username:</b> {('@'+uname) if uname else 'Отсутствует'}\n"
...
async def grant_all_do(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        f"Выдано: {len(ids)} — Пользователям\n"
        f"Уведомлено: {notified} — Пользователей"
    )
...
async def revoke_all_do(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        f"Забран: {len(revoked)} — Пользователей\n"
        f"Уведомлено: {notified} — Пользователей"
    )
    await safe_edit(call, text, access_panel_kb())
    await cb_answer(call)
...
async def list_no_access(call: CallbackQuery):
    ...
    for uid, uname in rows:
        text += f"Пользователь | <code>{uid}</code> — {('@'+uname) if uname else 'Отсутствует'}\n"
    await safe_edit(call, text, access_panel_kb())
...
async def list_admins(call: CallbackQuery):
    ...
    text = tg(T_LIST, "🧑‍💻") + " <b>Список админов</b>\n━━━━━━━━━━━━━━━━\n"
    for aid in admins:
        role = "Владелец" if aid == OWNER_ID else "Админ"
        text += f"<b>{role}:</b> {aid}\n"
...
async def wd_list(call: CallbackQuery):
    ...
    for wid, uid, amount, uname in rows:
        uname_s = f"@{uname}" if uname else "Отсутствует"
        if st == "pending":
