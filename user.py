...
from database import (
    ...
    ensure_user_record, get_price, count_queue, CAT_REG, CAT_NEW, CAT_LABEL, set_subscribed, slots_left, MAX_SLOTS
)
...
CHANNEL_ID = "@bear_afisha"
CHANNEL_URL = "https://t.me/bear_afisha"
...
def fmt_username(username) -> str:
    """Юзернейм для отображения: @name или Отсутствует."""
...
async def check_sub_callback(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if await is_admin(call.from_user.id) or await is_subscribed(bot, call.from_user.id):
        ...
        await call.message.answer(
            tg(T_OK, "✅") + " <b>Подписка подтверждена</b>",
            parse_mode="HTML",
        )
        ...
        await call.message.answer(
            tg(T_ERR, "🚫") + " <b>Подписка не найдена</b>",
            parse_mode="HTML",
        )
        await cb_answer(call)
        try:
...
async def public_queue_handler(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        f"{tg(T_QUEUE, '📋')} <b>Всего номеров в очереди:</b> {total}"
    )
...
async def submit_menu(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    if used >= mx:
        ...
        await call.message.answer(
            ...
            f"Все слоты заняты: <code>{used}</code>/<code>{mx}</code>.\n"
...
async def withdraw_start(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━━━\n"
            f"{tg(T_PAY, '💵')} Введите сумму вывода:"
        )
...
async def withdraw_amount(msg: Message, state: FSMContext, bot: Bot):
    ...
    await msg.answer(
        ...
        f"{tg(T_PAY, '💰')} <b>Баланс:</b> ${balance:.2f}\n"
        f"Останется: <code>${balance - amount:.2f}</code>",
...
async def withdraw_user_confirm(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    for admin_id in admins:
        try:
            await bot.send_message(
                admin_id,
                tg(T_PAY, "💰") + " <b>Новая заявка на вывод</b>\n"
                "━━━━━━━━━━━━━━━━\n"
                f"<b>От:</b> {uname}\n"
